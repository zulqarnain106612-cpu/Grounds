...
| [ ] | `phase0/ci-pipeline-and-wiring` | ci | *Gap-fill* — CI turnaround, PR status reporting, and tests for the seams between components | ADR-010 | `pr-status-comment` posts annotations on a red PR; architecture and pipeline suites green | `phase0_ci_pipeline_and_wiring` |
